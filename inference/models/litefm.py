...
class VisionTransformer(nn.Module):
    ...
    def infer_deploy(self, x, stage="pre", block_idx=0):
        if stage == "pre":
            B = x.shape[0]
            x = self.patch_embed(x)
            cls_tokens = self.cls_token.expand(B, -1, -1)
            ...
            for i, blk in enumerate(self.blocks[:block_idx+1]):
                x = blk(x)
...
if __name__ == "__main__":
    ...
    model = custom_vit_small_patch16_224(device, '/nfs/usrhome/xhuangbs/ycaibt/CPath/PrePATH/models/ckpts/LiteFM.pth', proj_dim=1024)
