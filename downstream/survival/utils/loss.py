...
def define_loss(args):
    if args.loss == "aps":
        loss = ScoreLoss(args.temperature)
    elif args.loss == "ce_surv":
...
class ScoreLoss(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits):
        assert student_logits.shape == teacher_logits.shape, (
            f"student_logits and teacher_logits must have the same shape: {student_logits.shape} != {teacher_logits.shape}"
        )
        teacher_prob = F.softmax(teacher_logits / self.temperature, dim=0)
        student_log_prob = F.log_softmax(student_logits / self.temperature, dim=0)
        return -torch.sum(teacher_prob * student_log_prob)
